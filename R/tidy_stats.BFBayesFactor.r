...
tidy_stats.BFBayesFactor <- function(x, args = NULL) {
  ...
  if (nrow(bayes_factors) == 1) {
    ...
    statistics <- add_statistic(
      statistics, "BF10", bayes_factors$bf, "BF",
      "10"
    )
    statistics <- add_statistic(
      statistics, "BF01", 1 / bayes_factors$bf, "BF",
      "01"
    )
    statistics <- add_statistic(
      statistics, "proportional error",
    ...
    for (i in seq_len(nrow(bayes_factors))) {
      ...
      statistics <- add_statistic(
        statistics, "BF10", bayes_factors$bf[i], "BF",
        "10"
      )
      statistics <- add_statistic(
        statistics, "BF01", 1 / bayes_factors$bf[i],
        "BF", "01"
      )
      statistics <- add_statistic(
        statistics, "proportional error",
