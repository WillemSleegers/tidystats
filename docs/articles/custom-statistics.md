...
[page](https://github.com/WillemSleegers/tidystats) and create an
[issue](https://github.com/WillemSleegers/tidystats/issues). This is the
preferred method because it is easy to paste code snippets and to ask
follow-up questions. For alternative ways to contact me, see the
...
return a standard matrix, rather than an object specific to the
...
what it is working with and extract the statistics.

Thankfully, we can still add statistics from
[`confint()`](https://rdrr.io/r/stats/confint.html) to a list via
[`add_stats()`](https://willemsleegers.github.io/tidystats/reference/add_stats.md),
using the `class` argument. We can specify that the statistics are from
the [`confint()`](https://rdrr.io/r/stats/confint.html) function by
saying `class = "confint"`.
...
please contact me to let me know that I should add support for it.
