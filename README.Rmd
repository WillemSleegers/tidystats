...
`tidystats` is an R package aimed at sharing and reporting the output of statistical models. `tidystats` extracts the output (e.g., *t*-values, *p*-values) of statistical models (e.g., *t*-tests, regression models) and combines them into a structured file. This file can then be used to report the statistics in a manuscript or shared with others so that they can extract the statistics (e.g., for meta-analyses).
